...
const FranceMap = ({
    ...
    const renderDomTomRegion = (region) => {
        ...
        const transform = region.transform
            ? `translate(${region.transform.translateX}, ${region.transform.translateY}) scale(${region.transform.scale})`
            : undefined;
            ...
            <G key={region.id}>
                <G transform={transform}>
                    <Path
                        ...
                        fill={getRegionFill(region.id)}
                        stroke="#FFFFFF"
                        strokeWidth={isSelected ? 2 / (region.transform?.scale || 1) : 1.5 / (region.transform?.scale || 1)}
                        strokeLinejoin="round"
                        onPressIn={() => setPressedRegion(region.id)}
                        onPressOut={() => {
                            setPressedRegion(null);
